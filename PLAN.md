...
**노쇼 패널티 시스템 — 2026-06-14**
- `PATCH /api/applications/[id]/cancel`: 기사·소장 모두 배차 취소 가능, 취소한 본인에게 패널티 부여
- `profiles.penalty_count` 증가, `banned_until` 자동 세팅 (5회→+3일, 10회→+7일)
- `is_certified` 자동화: 리뷰 5건↑ + 평점 4.5↑ 시 `is_certified = true` 자동 전환
- 지원 API (`POST /api/applications`): `banned_until` 활성 시 403 차단 (기사)
- 일감 등록 API (`POST /api/jobs`): `banned_until` 활성 시 403 차단 (소장)
- `JobApplyButton`: 밴 중일 때 "🚫 패널티 누적 — 지원 제한 중" 배너 표시
- 일감 등록 페이지: 밴 중일 때 폼 대신 제한 안내 + 마이페이지 링크
- 소장 패널티 뱃지: 일감 카드 · 일감 상세 소장 이름 옆 주황 배지 표시
- 기사 패널티 뱃지: 지원자 목록 카드 이름 옆 + 마이페이지 프로필 카드
- 프로필 링크: 소장 일감 카드 이름 클릭 → `/profiles/[id]`, 지원자 카드 "프로필 →" 링크
- `ApplicantCard` 클라이언트 컴포넌트 전환 (중첩 anchor 방지)

**QA 체크리스트 — 노쇼 패널티 시스템**

| # | 시나리오 | 역할 | 예상 결과 |
|---|---------|------|---------|
| 1 | 기사가 accepted 지원 취소 | 기사 | `cancelled_by_driver` 상태, 기사 penalty_count +1, 소장에게 취소 알림 |
| 2 | 소장이 accepted 배차 취소 | 소장 | `cancelled_by_manager` 상태, 소장 penalty_count +1, 기사에게 취소 알림 |
| 3 | 기사 penalty_count = 5 도달 시 취소 | 기사 | banned_until = 취소 시각 +3일 자동 세팅 |
| 4 | 기사 penalty_count = 10 도달 시 취소 | 기사 | banned_until = 취소 시각 +7일 자동 세팅 |
| 5 | 소장 penalty_count = 5 도달 시 취소 | 소장 | banned_until = 취소 시각 +3일 자동 세팅 |
| 6 | 소장 penalty_count = 10 도달 시 취소 | 소장 | banned_until = 취소 시각 +7일 자동 세팅 |
| 7 | 밴 중인 기사가 일감 지원 시도 | 기사 | 지원 버튼 자리에 "🚫 패널티 누적 — 지원 제한 중 (N월 N일까지)" 배너 |
| 8 | 밴 중인 기사가 지원 API 직접 호출 | 기사 | 403 + "패널티 누적으로 N월 N일까지 지원이 제한됩니다." |
| 9 | 밴 중인 소장이 일감 등록 페이지 접근 | 소장 | 폼 대신 제한 안내 + 마이페이지 링크 표시 |
| 10 | 밴 중인 소장이 일감 등록 API 직접 호출 | 소장 | 403 + "패널티 누적으로 N월 N일까지 일감 등록이 제한됩니다." |
| 11 | 일감 카드에서 패널티 있는 소장 확인 | 기사 | 소장 이름 옆 주황 "패널티 N" 뱃지 표시 |
| 12 | 일감 상세에서 패널티 있는 소장 확인 | 기사 | 소장 정보 섹션 이름 옆 주황 "패널티 N회" 뱃지 표시 |
| 13 | 지원자 목록에서 패널티 있는 기사 확인 | 소장 | 기사 이름 옆 주황 "패널티 N회" 뱃지 표시 |
| 14 | 마이페이지에서 내 패널티 확인 | 기사/소장 | 프로필 카드 이름 옆 주황 "패널티 N회" 뱃지 표시 |
| 15 | 일감 카드 소장 이름 클릭 | 기사 | 카드 클릭(일감 상세)과 독립적으로 `/profiles/[소장id]` 이동 |
| 16 | 지원자 카드 "프로필 →" 클릭 | 소장 | 상세 보기 클릭과 독립적으로 `/profiles/[기사id]` 이동 |
| 17 | accepted 상태가 아닌 지원 취소 시도 | 기사/소장 | 400 + "배차 확정 상태에서만 취소할 수 있습니다." |
| 18 | 다른 사람의 지원 취소 시도 | 기사/소장 | 403 + "권한이 없습니다." |
