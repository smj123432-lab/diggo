...
랜딩, 로그인, 회원가입, 일감 목록, 일감 상세, 일감 등록, 내 일감 목록, 지원자 목록, 지원자 상세, 마이페이지, 프로필 수정, 장부, 내 지원 목록, 받은 평가, 관리자 대시보드
...
```
pending (지원) → reviewing (검토중, 채팅 오픈) → accepted (수락) or rejected (거절)
```
...
| 순서 | 작업                                  | 기간  |
| ---- | ------------------------------------- | ----- |
| 1    | 프로젝트 세팅 + Supabase 테이블 + RLS | 1일   |
| 2    | 인증 (로그인 / 회원가입 / 역할 선택)  | 1~2일 |
| 3    | 일감 목록 + 필터 + 무한스크롤         | 2일   |
| 4    | 일감 등록 폼 + 카카오맵               | 1~2일 |
| 5    | 일감 상세 + 지원 신청                 | 1일   |
| 6    | 지원자 관리 (소장)                    | 1일   |
| 7    | 장부 탭 (달력 + 지출 입력)            | 2~3일 |
| 8    | 마이페이지 + 상호 평가                | 1~2일 |
| 9    | 버그 수정 + 마무리                    | 2~3일 |
...
- 자격증 인증 플로우 (Supabase Storage + 관리자 승인)
- 알림
- 장비 여러 대 운용 (담당 기사 지정)
- 관리자 세부 페이지
